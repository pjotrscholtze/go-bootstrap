...
func TestCollapse(t *testing.T) {
	...
	html, err := Collapse(Button(BsBtnPrimary, BsBtnNormal, "text", BsBtnLg, BsBtnStateDisabled, nil).(*htmlwrapper.HTMLElm), "id", "target").AsHTML()
	if err != nil {
...
func TestCollapseIdAndTarget(t *testing.T) {
	// Collapse(btn *htmlwrapper.HTMLElm, id, target string)
	html, err := Collapse(Button(BsBtnPrimary, BsBtnNormal, "text", BsBtnLg, BsBtnStateDisabled, nil).(*htmlwrapper.HTMLElm), "fdsa", "asdf").AsHTML()
	if err != nil {
		t.Fatalf("Failed to make HTML! %s", err)
	}
